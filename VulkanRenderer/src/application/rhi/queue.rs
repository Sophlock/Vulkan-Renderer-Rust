...
impl QueueFamilyIndices {
    ...
    pub fn find_queue_indices(physical_device: &PhysicalDevice, surface: &Surface) -> Self {
        let queues = physical_device.queue_family_properties().iter().zip(0u32..);
        let ideal_queues = queues.clone().filter(|(prop, index)| {
        ...
        ideal_queues
            ...
            .unwrap_or_else(|| QueueFamilyIndices {
                graphics_family: Self::find_queue_index_with_flag(
                    queues.clone(),
                    QueueFlags::GRAPHICS,
                )
                .unwrap(),
                present_family: Self::find_present_index(queues.clone(), &physical_device, surface)
                    .unwrap(),
                compute_family: Self::find_queue_index_with_flag(
                    queues.clone(),
                    QueueFlags::COMPUTE,
                )
                .unwrap(),
    ...
    fn find_queue_index_with_flag<'a>(
        queues: impl IntoIterator<Item = (&'a QueueFamilyProperties, u32)>,
        flag: QueueFlags,
    ) -> Option<u32> {
        queues
            .into_iter()
...
impl QueueCollection {
    pub fn new(queues: Vec<Arc<Queue>>, queue_family_indices: &QueueFamilyIndices) -> Self {
        Self {
            graphics_queue: Self::find_queue_of_family(
                &queues,
    ...
    fn find_queue_of_family(queues: &Vec<Arc<Queue>>, queue_family_index: u32) -> Arc<Queue> {
        queues
            .into_iter()
            .filter(|queue| queue.queue_family_index() == queue_family_index)
            .last()
            .unwrap()
            .clone()
    }
