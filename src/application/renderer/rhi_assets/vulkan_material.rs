...
use shader_slang::{ComponentType, Downcast};
use std::{
    ops::Deref,
    sync::Arc
};
...
impl VulkanMaterial {
    pub fn new(compiler: &SlangCompiler, device: &Arc<Device>, render_pass: Arc<RenderPass>) -> shader_slang::Result<Self> {
        ...
        let frag_spriv = linked.entry_point_code(1, 0)?;
        let shader_object_layout = ShaderObjectLayout::new(device);

        ShaderObject::new(device, render_pass, shader_object_layout, bytes_to_words(vert_spirv.as_slice()).unwrap().deref(), bytes_to_words(frag_spriv.as_slice()).unwrap().deref());
